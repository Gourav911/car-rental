...
function TopBanner() {
  return (
    <div className="top-banner" style={{ background: '#0f172a', color: '#fff', fontSize: '12px', fontWeight: '600', padding: '8px 20px', textAlign: 'center', borderBottom: '1px solid rgba(255,255,255,0.1)', position: 'relative', zIndex: 50 }}>
      🚘 Need a rental car for your journey? Get the best rates at{' '}
      <a href="https://carrentaldesk.net" target="_blank" rel="noopener noreferrer" style={{ color: '#34d399', textDecoration: 'underline', fontWeight: '700', marginLeft: '4px' }}>
        CarRentalDesk.net
      </a>
    </div>
  );
    ...
    <header className="nav">
      <div className="nav-inner">
        ...
        <div className="nav-controls">
          ...
          <a href="https://carrentaldesk.net" target="_blank" rel="noopener noreferrer" className="nav-cta desktop-only" style={{ background: '#059669', textDecoration: 'none' }}>
            Rent a Car <Car size={15} />
          </a>
      ...
      <AnimatePresence>
        {open && (
          <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: 'auto', opacity: 1 }} exit={{ height: 0, opacity: 0 }} className="mobile-nav" style={{ display: 'flex', flexDirection: 'column', gap: '12px', padding: '16px' }}>
            ...
            <a href="https://carrentaldesk.net" target="_blank" rel="noopener noreferrer" style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '8px', background: '#059669', color: '#fff', padding: '12px', borderRadius: '12px', fontWeight: '800', textDecoration: 'none', fontSize: '13px' }}>
              Rent a Car <Car size={15} />
            </a>
          </motion.div>
        )}
...
function BookingModal({ flight, activeCurrency, onClose, onBookingSuccess }) {
  ...
  const handleComplete = async (e) => {
    ...
    const bookingDetails = {
      ...
      phone: phone || 'N/A',
      bookingNumber: pnr,
      flightDetails: `${flight.airline} ${flight.flightNumber} (${flight.fromCity} [${flight.fromCode}] ➔ ${flight.toCity} [${flight.toCode}])`,
      departureDate: flight.depDate || flight.departureTime || 'N/A',
      cabinClass: flight.cabinClass || 'Economy',
      fareTier: fareTier,
      totalAmount: formatPrice(calculateTotalUSD(), activeCurrency)
    };

    try {
      await submitToGoogleSheets(bookingDetails);
    } catch (err) {
      console.error('Failed to submit booking to Google Sheets:', err);
    }
  ...
  return (
    <main id="home">
      ...
      <section className="section container">
        <div className="section-heading centered">
          <div className="eyebrow">Why Skyline Journeys</div>
      ...
      <section className="section faq-section container">
        ...
        <div className="faq-list">
          {[
            ['How does Skyline Journeys help me find flights?', 'We surface thoughtful flight ideas and clear fare options with live client-side currency switching and reservation preview.'],
...
function StaticPage({ type, setPage }) {
  ...
  const data = {
    about: ['Travel should feel like the beginning of something good.', 'Skyline Journeys was created for curious people who value both the destination and the way they get there. We believe great travel starts with a clear mind, an open map, and someone in your corner.', 'We make the early stages of planning feel more human: less frantic comparison, more helpful perspective.'],
    contact: ['Let’s plan a little magic.', 'Whether you have a route in mind or just a feeling you want to follow, we’d love to hear from you.', 'hello@skylinejourneys.com'],
    ...
    privacy: ['Privacy, plainly put.', 'Skyline Journeys respects your privacy. This demonstration site does not create accounts, collect payment details, or track browser fingerprints. Contact submissions are held in local state only.', 'We do not sell personal information.'],
    terms: ['Terms & conditions.', 'This website is an informational showcase of Skyline Journeys. Fares and flight schedules are simulated for demonstration purposes.', 'Please confirm travel details with relevant airlines prior to travel.']
  };
  ...
  return (
    <main className="inner-page">
      <div className="container">
        <div className="eyebrow">Skyline Journeys</div>
        <h1>{title}</h1>
            ...
            {type === 'contact' && (
              <div style={{ marginTop: '24px' }}>
                ...
                <div className="contact-details" style={{ marginTop: '24px' }}>
                  <span><Mail /> hello@skylinejourneys.com</span>
  ...
  return (
    <footer>
      ...
      <div className="container footer-bottom">
        <span>© 2026 Skyline Journeys. All rights reserved.</span>
        <div>
...
function App() {
  ...
  const [wishlist, setWishlist] = useState(() => {
    try {
      const saved = localStorage.getItem('skylinejourneys_wishlist');
  ...
  useEffect(() => {
    try {
      localStorage.setItem('skylinejourneys_wishlist', JSON.stringify(wishlist));
    } catch (err) {
      console.error(err);
    }
  }, [wishlist]);
